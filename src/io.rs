#![cfg(not(loom))]
// I honestly think we should brain on something better than this
...
impl Builder {
    ...
    pub fn build_with_handle(&mut self) -> (Mock, Handle) {
        let (inner, handle) = Inner::new(self.actions.clone(), self.logger.clone());
...
impl Inner {
    fn new(actions: VecDeque<Action>, logger: Logger) -> (Inner, Handle) {
    ...
    fn read(&mut self, dst: &mut [u8]) -> io::Result<usize> {
        debug!(self.logger, "read, actions = {:?}", self.actions);
        match self.action() {
    ...
    fn write(&mut self, mut src: &[u8]) -> io::Result<usize> {
        debug!(
            self.logger,
            "write, actions = {:?}, src = {:?}", self.actions, src
        );
        let mut ret = 0;
            ...
            match self.actions[i] {
                Action::Write(ref mut expect) => {
                    ...
                    if &src[..n] != &expect[..n] {
                        println!(
                            "Found {}, expected {}",
                            std::str::from_utf8(&src[..n]).unwrap(),
                            std::str::from_utf8(&expect[..n]).unwrap()
                        );
                    }
    ...
    fn action(&mut self) -> Option<&mut Action> {
        loop {
            ...
            debug!(self.logger, "action, action = {:?}", self.actions[0]);
            match self.actions[0] {
                ...
                Action::Wait(ref mut dur) => {
                    debug!(self.logger, "action, Wait, waiting = {:?}", self.waiting);
        ...
        debug!(self.logger, "action, returning {:?}", self.actions[0]);
